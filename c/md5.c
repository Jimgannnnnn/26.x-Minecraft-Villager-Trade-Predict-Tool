...
static void md5_update(MD5_CTX* ctx, const uint8_t* input, size_t len) {
    ...
    if (len >= part) {
        ...
        for (size_t i = part; i + 63 < len; i += 64) md5_transform(ctx->state, input + i);
        idx = 0;
    }
    if (len > part) input += (len - (len % 64)) - part, len %= 64;
    memcpy(ctx->buffer + idx, input, len);
}

static void md5_final(MD5_CTX* ctx, uint8_t digest[16]) {
    uint8_t bits[8];
    for (int i = 0; i < 8; i++) bits[i] = (uint8_t)(ctx->count[i>>2] >> ((i&3)<<3));
    size_t idx = (ctx->count[0] >> 3) & 0x3F;
    static const uint8_t padding[64] = {0x80};
    md5_update(ctx, padding, idx < 56 ? 56 - idx : 120 - idx);
...
void md5_hash_bytes(const char* input, int len, uint8_t digest[16]) {
    MD5_CTX ctx;
    md5_init(&ctx);
    md5_update(&ctx, (const uint8_t*)input, len);
    md5_final(&ctx, digest);
}

Seed128 seed_from_hash(const char* identifier) {
    uint8_t digest[16];
    md5_hash_bytes(identifier, (int)strlen(identifier), digest);
    Seed128 s;
    s.lo = ((uint64_t)digest[7] << 56) | ((uint64_t)digest[6] << 48) |
           ((uint64_t)digest[5] << 40) | ((uint64_t)digest[4] << 32) |
           ((uint64_t)digest[3] << 24) | ((uint64_t)digest[2] << 16) |
           ((uint64_t)digest[1] << 8)  | (uint64_t)digest[0];
    s.hi = ((uint64_t)digest[15] << 56) | ((uint64_t)digest[14] << 48) |
           ((uint64_t)digest[13] << 40) | ((uint64_t)digest[12] << 32) |
           ((uint64_t)digest[11] << 24) | ((uint64_t)digest[10] << 16) |
           ((uint64_t)digest[9] << 8)  | (uint64_t)digest[8];
    return s;
}
